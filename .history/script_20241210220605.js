...
function updateTimeRange(timeContent, startTime, endTime) {
    ...
    if (startTime && endTime) {
        ...
        if (duration > 1) {
            ...
            contentWrapper.classList.remove('multi-hour');
            delete contentWrapper.dataset.duration;
        }
...
function handleTimeContentResize(timeContent, hour) {
    ...
    resizeHandle.addEventListener('mousedown', (e) => {
        ...
        function updateTimeIndicator(height) {
            const slots = Math.round((height - 30) / 30);
            const endHour = Math.min(parseInt(hour) + slots, 24);
            timeIndicator.textContent = `${hour}:00 - ${endHour}:00`;
            
            const rect = timeContent.getBoundingClientRect();
            timeIndicator.style.top = `${rect.bottom}px`;
            timeIndicator.style.left = `${rect.right + 10}px`;
        }
        ...
        function onMouseMove(e) {
            ...
            const deltaY = e.clientY - startY;
            const newHeight = Math.max(30, Math.min(startHeight + deltaY, 180));
            timeContent.style.height = `${newHeight}px`;
            
            updateTimeIndicator(newHeight);
