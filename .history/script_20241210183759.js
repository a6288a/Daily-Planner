...
// 修改 initializeTimeline 函数中创建时间内容的部分
function createTimeContent(hour) {
    ...
    expandBtn.className = 'time-expand';
    expandBtn.textContent = '▼';
    expandBtn.title = '设置时间范围';
    
    // 添加拖动手柄
    const dragHandle = document.createElement('span');
    dragHandle.className = 'time-drag-handle';
    dragHandle.innerHTML = '⋮⋮';
    dragHandle.title = '长按拖动';
    
    timeContent.appendChild(dragHandle);
    timeContent.appendChild(timeRange);
    timeContent.appendChild(contentDiv);
    timeContent.appendChild(expandBtn);
    ...
    expandBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        showTimeRangePicker(timeContent, hour);
    });
...
// 修改时间轴内容的拖拽处理
function initializeTimeContentDrag(timeContent) {
    ...
    dragHandle.addEventListener('mousedown', (e) => {
        ...
        longPressTimer = setTimeout(() => {
            ...
            // 创建拖拽时的视觉反馈
            const ghost = timeContent.cloneNode(true);
            ...
            function onMouseMove(e) {
                ...
                // 查找目标时间槽
                const targetSlot = findTargetTimeSlot(e.clientY);
                if (targetSlot) {
                    highlightTimeSlot(targetSlot);
                }
            }
            ...
            function onMouseUp(e) {
                if (isDragging) {
                    const targetSlot = findTargetTimeSlot(e.clientY);
                    if (targetSlot) {
                        moveTimeContent(timeContent, targetSlot);
                    }
...
// 添加高亮时间槽的函数
function highlightTimeSlot(slot) {
    removeHighlights();
    slot.classList.add('time-slot-target');
}

// 移除所有高亮的函数
function removeHighlights() {
    document.querySelectorAll('.time-slot-target').forEach(slot => {
        slot.classList.remove('time-slot-target');
    });
}

// 移动时间内容的函数
function moveTimeContent(sourceContent, targetSlot) {
    const sourceHour = sourceContent.dataset.hour;
// 添加日期相关变量
let currentDate = new Date();
let currentDateKey = formatDateKey(currentDate);
