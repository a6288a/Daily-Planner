    ...
    header.addEventListener('mousedown', (e) => {
        ...
        function onMouseMove(e) {
            ...
            frogTasks.forEach(task => {
                ...
                if (e.clientX >= taskRect.left && e.clientX <= taskRect.right &&
            ...
            if (isDragging) {
                ...
                frogTasks.forEach((task, index) => {
                    ...
                    if (e.clientX >= taskRect.left && e.clientX <= taskRect.right &&
                        ...
                        // 清除该便签在其他青蛙位置的记录
                        dailyData[currentDateKey].frogTasks = dailyData[currentDateKey].frogTasks.map(task => 
                            task && task.noteId === element.dataset.id ? null : task
                        );
...
function initializeFrogTasks() {
    ...
    frogTasks.forEach((task, taskIndex) => {
        ...
        task.addEventListener('drop', (e) => {
            ...
            // 检查便签是否存在
            const noteExists = dailyData[currentDateKey].freeNotes.some(note => note.id === noteId);
...
function formatDate(date) {
    ...
    const weekday = '日��二三四五六'.charAt(date.getDay());
...
function makeNoteDraggableToFrogs(note) {
    ...
    header.addEventListener('mousedown', (e) => {
        ...
        function onDragEnd() {
            ...
            note.removeEventListener('dragstart', onDragStart);
            note.removeEventListener('dragend', onDragEnd);
        }
...
// 添加清理青蛙任务的函数
function cleanupFrogTasks() {
    // 获取所有存在的便签ID
    const existingNoteIds = dailyData[currentDateKey].freeNotes.map(note => note.id);
    
    // 清理不存在便签的青蛙任务
    dailyData[currentDateKey].frogTasks = dailyData[currentDateKey].frogTasks.map(task => {
        if (task && !existingNoteIds.includes(task.noteId)) {
            return null;
        }
        return task;
    });
