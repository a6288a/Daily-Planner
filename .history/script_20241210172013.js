...
function makeDraggable(element) {
    ...
    header.addEventListener('mousedown', (e) => {
        ...
        function onMouseMove(e) {
            ...
            if (!isDragging && Math.abs(e.clientX - startX) < 5 && Math.abs(e.clientY - startY) < 5) return;
...
function updateFrogTasks() {
    ...
    dailyData[currentDateKey].frogTasks.forEach((task, index) => {
        if (task && task.content) {
            taskContents[index].textContent = task.content;
        } else {
            taskContents[index].textContent = '';
        }
    });
    saveToLocalStorage();
}
